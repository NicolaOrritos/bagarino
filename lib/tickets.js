...
function handleRequestsBasedTicketResponse(ticket_base, res, next, light)
{
    ...
    {
        throw new Error('Missing "ticket_base" parameter');
    }
}
...
exports.payload = function(req, res, next)
{
    ...
    if (ticket_base)
    {
        ...
        client.exists(CONST.VALID_PREFIX + ticket_base, (err, exists) =>
        {
            ...
            {
                client.hgetall(CONST.VALID_PREFIX + ticket_base, (err, metadata) =>
                {
                    if (err)
                    {
                    ...
                    else
                    {
                        ...
                        // Now decrease the ticket counters (if bandwidth- or requests-based):
                        metadata.policy = JSON.parse(metadata.policy);

                        if (metadata.policy.requests_based)
                        {
                            handleRequestsBasedTicketResponse(ticket_base, null, next, req.query.light);

                            // TODO: Payloads can't be used for auto-renewables...
                        }
                        else if (metadata.policy.bandwidth_based)
                        {
                            handleBandwidthTicketResponse(ticket_base, null, next, req.query.light);
                        }
                    }
                });
            }
