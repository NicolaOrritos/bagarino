        ...
        else
        {
            ...
            for (let a=0; a<count; a++)
            {
                ...
                if (policy.time_based)
                {
                    ...
                    if (policy.payload)
                    {
                        client.hset(valid_ticket, 'payload', req.body);
                    }
    ...
    if (ticket_base)
    {
        ...
        client.exists(CONST.VALID_PREFIX + ticket_base, (err, exists) =>
        {
            ...
            else
            {
                ...
                client.exists(CONST.EXPIRED_PREFIX + ticket_base, (err, expired) =>
                {
    ...
    if (ticket_base)
    {
        client.exists(CONST.VALID_PREFIX + ticket_base, (err, exists) =>
        {
            ...
            else
            {
                ...
                client.exists(CONST.EXPIRED_PREFIX + ticket_base, (err, expired) =>
                {
                    global.log.debug('[tickets.policy] expired returned: %s', expired);
                    global.log.debug('[tickets.policy] error was: %s', err);
        ...
        client.exists(CONST.VALID_PREFIX + ticket_base, (err, exists) =>
        {
            ...
            else
            {
                ...
                client.exists(CONST.EXPIRED_PREFIX + ticket_base, (err, expired) =>
                {
                    ...
                    global.log.debug('[tickets.expire] error was: %s', err);
    ...
    if (ticket_base)
    {
        ...
        client.exists(CONST.VALID_PREFIX + ticket_base, (err, exists) =>
        {
            ...
            else
            {
                ...
                client.exists(CONST.EXPIRED_PREFIX + ticket_base, (err, expired) =>
                {
                    global.log.debug('[tickets.payload] expired returned: %s', expired);
                    global.log.debug('[tickets.payload] error was: %s', err);
