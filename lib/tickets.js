...
function handleRequestsBasedTicketResponse(ticket_base, res, light)
{
    client.select(REDIS_DB, function()
    {
        client.hget(CONST.VALID_PREFIX + ticket_base, 'policy', function(err, policy_str)
        {
            ...
            if (policy_str)
            {
                ...
                {
                    var isLight = (light === true || light === 'true');
...
function handleBandwidthTicketResponse(ticket_base, res, light)
{
    client.select(REDIS_DB, function()
    {
        client.hget(CONST.VALID_PREFIX + ticket_base, 'policy', function(err, policy_str)
                    ...
                    client.hget(CONST.VALID_PREFIX + ticket_base, 'policy', function(err, policy_str)
                    {
                        if (policy_str)
                        {
                            ...
                            if (can_go_on)
                            {
                                ...
                                {
                                    handleRequestsBasedTicketResponse(ticket_base, res, req.query.light);
                                }
                                else if (policy.manual_expiration)
                                {
                                ...
                                {
                                    handleBandwidthTicketResponse(ticket_base, res, req.query.light);
                                }
