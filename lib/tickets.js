...
function writeExpired(ticket, expiration)
{
    var expired_ticket = ticket;

    if (expired_ticket.indexOf && expired_ticket.indexOf(CONST.EXPIRED_PREFIX) === -1)
    {
        expired_ticket = CONST.EXPIRED_PREFIX + expired_ticket;
    }

    if (expired_ticket)
    {
        client.hset(expired_ticket, 'content', CONST.EXPIRED_TICKET);
        client.hset(expired_ticket, 'created', Date().getTime());

        if (!isNaN(expiration))
        {
            client.expire(expired_ticket, expiration);
        }
    }
    else
    {
        throw new Error('Could not set expired counterpart: ticket was empty (value: "%s")', expired_ticket);
    }
}

function writeValid(ticket, policy, expiration)
{
    var valid_ticket = ticket;

    if (valid_ticket.indexOf && valid_ticket.indexOf(CONST.VALID_PREFIX) === -1)
    {
        valid_ticket = CONST.EXPIRED_PREFIX + valid_ticket;
    }
    ...
    if (valid_ticket)
    {
        ...
        {
            // Not a string...
            if (policy.trim === undefined || policy.trim === null)
            {
                policy = JSON.stringify(policy);
            }

            client.hset(valid_ticket, 'content', CONST.VALID_TICKET);
            client.hset(valid_ticket, 'policy',  policy);

            if (!isNaN(expiration))
            {
                client.expire(valid_ticket, expiration);
            }
        }
        else
        {
            throw new Error('Could not set valid counterpart: policy was empty (value: "%s")', policy);
        }
    }
    else
    {
        throw new Error('Could not set valid counterpart: ticket was empty (value: "%s")', valid_ticket);
    }
}
